...
with get_session('airflow_mysql_mexico') as db_session:
    def depurate():
        logging.debug("Comenzando borrado de datos actuales")
        delete_inactives(db_session=db_session)
        logging.debug("Borrado exitoso, comenzando carga de nuevos datos")

    def comisiones_loader():
        logging.info("Ejecutando ComisionesLoader")
        copy_base_info_from_clients(db_session=db_session)
        logging.debug("Carga inicial [OK]")
        ...
        # Calculo de activos
        logging.debug("Comenzando calculo de activos")
        ...
        logging.debug("Cambiar estados de registros")
...
start_task >> depurate_table >> end_task
